function [t, u] = BDF3_solver(A, f, dfdu, u_init, tspan, h)
% Solve u' + A u = f(u,t) using fixed-order BDF3.
% u_init: N x 3 matrix with columns [u(t0-2h), u(t0-h), u(t0)]
if nargin < 3 || isempty(dfdu)
    has_jac = false;
    dfdu = @(u,t) sparse(length(u), length(u));
else
    has_jac = true;
end
N = size(u_init,1);
if size(u_init,2) ~= 3
    error('u_init must be N x 3 matrix.');
end
t0 = tspan(1); tf = tspan(2);
t = t0:h:tf;
if t(end) ~= tf
    t(end) = tf;
    warning('Last step adjusted.');
end
nt = length(t);
u = zeros(N, nt);
u(:,1) = u_init(:,3);
% Fixed BDF3 coefficients
alpha_past = [-18/11, 9/11, -2/11];
beta = 6/11;
coeffs = alpha_past(end:-1:1)'; % For history * coeffs = sum_past
% Initial history: columns [u_{-2}, u_{-1}, u_{0}]
history = u_init;
for n = 1:nt-1
    curr_h = t(n+1) - t(n);
    sum_past = history * coeffs;
    U0 = u(:,n); % Initial guess: last known u
    options = optimoptions('fsolve', 'Display', 'off', 'MaxIterations', 100, 'TolFun', 1e-15);
    if has_jac
        options = optimoptions(options, 'SpecifyObjectiveGradient', true);
    end
    t_new = t(n+1);
    [U, ~, exitflag] = fsolve(@(U) resfun(U, sum_past, curr_h, t_new, A, f, dfdu, beta, has_jac, N), U0, options);
    if exitflag <= 0
        warning('fsolve failed at step %d.', n);
    end
    u(:,n+1) = U;
    % Update history: shift and add new U
    history = [history(:,2:3), U];
end
end
function [res, jac] = resfun(U, sum_past, h, t_new, A, f, dfdu, beta, has_jac, N)
K = -A * U + f(U, t_new);
res = U + sum_past - h * beta * K;
if nargout > 1 && has_jac
    Df = dfdu(U, t_new);
    jac = speye(N) + h * beta * (A - Df);
else
    jac = [];
end
end
